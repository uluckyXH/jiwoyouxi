...
const source = ['ArenaLayout.ets', 'ArenaWindow.ets'].map(name => readFileSync(new URL(name, base), 'utf8'))
...
function fixture({ folded = 1, foldable = true, delayed = false } = {}) {
  ...
  let regions = [{ left: 40, top: 500, width: 1200, height: 20 }];
  const host = {
  ...
  const display = { isFoldable: () => foldable, getFoldStatus: () => status,
    ...
    getCurrentFoldCreaseRegion: () => {
      ...
      return { creaseRects: regions };
    }
  };
  const context = { display, window, console: { warn: () => {} } };
  vm.runInNewContext(`${code}\nglobalThis.Observer=ArenaWindow;globalThis.layout=arenaLayout;`, context);
  ...
  return { observer, listeners, native, samples, writes,
    ...
    get limited() { const s = samples.at(-1); return context.layout(800, 1000, s.insets, s.creases).limited; },
    layoutAt: (width, height) => { const s = samples.at(-1); return context.layout(width, height, s.insets, s.creases); },
    setStatus: value => { status = value; },
    setRegions: value => { regions = value; },
    failReads: () => { avoidError = true; creaseError = true; },
    resolveLookup: () => lookup(host),
    attach: () => observer.attach({}, px => px / 2, (insets, creases, fold) => samples.push({ insets, creases, fold }),
      () => { backgrounds++; })
  };
}
{
...
}
{
  ...
  assert.equal(f.limited, false);assert.equal(f.last.creases.length, 0);
  assert.equal(f.layoutAt(390,844).limited,false);assert.equal(f.layoutAt(250,800).limited,true);
  f.observer.release();
...
}
{
  ...
  assert.equal(f.last.creases[0].x, 0);assert.equal(f.last.creases[0].y, 220);assert.equal(f.last.creases[0].height, 10);
  ...
  for (const status of [1,2,11,12]) {
    ...
    assert.equal(f.last.creases.length,0,'flat multi-fold panels ignore the closed hinge');
    assert.equal(f.layoutAt(390,844).limited,false);
  }
  f.native.get('foldStatusChange')(1);assert.equal(f.limited, false);assert.equal(f.last.creases.length, 0);
  f.setStatus(2);f.listeners.get('windowEvent')(1);assert.equal(f.last.fold.status,2, 'foreground rechecks missed status');
  assert.equal(f.limited,false);assert.equal(f.layoutAt(250,800).limited,true);
...
{
  const f=fixture({folded:21});
  f.setRegions([{left:820,top:60,width:20,height:1720},{left:1660,top:60,width:20,height:1720}]);
  await f.attach();assert.equal(f.last.creases.length,2);
  assert.equal(f.last.creases[0].x,390);assert.equal(f.last.creases[1].x,810);
  const layout=f.layoutAt(1220,860);assert.equal(layout.limited,false);
  for(const c of f.last.creases)assert.ok(layout.board.x+layout.board.width<=c.x||layout.board.x>=c.x+c.width);
  f.observer.release();
  console.log('PASS all window-local creases are preserved and the chosen pane avoids both hinges');
}
console.log('\n6 native-window cases passed; physical fold sensors still require device verification.');
