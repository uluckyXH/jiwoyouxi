...
vm.runInNewContext(stripTypeScriptTypes(source, { mode: 'transform' }) + '\nglow2048NextTest();', context);
const assetRoot = resolve(root, 'entry/src/main/resources/rawfile/gamesNext/glow2048');
for (const effect of ['slide','merge','bright','tap','blocked','milestone','win','end']) {
  ...
  assert.ok(peak >= 0.16 && peak < 0.85, `${effect}: audible amplitude with headroom, peak=${peak}`);
}
