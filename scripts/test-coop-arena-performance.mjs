...
const scene={session:'arena-test',label:'据点争夺 / 花盆迷阵',seed:789,faction:1,initial:'20/20/20',window:'390×844',board:366,density:3.5,fold:2,dark:false,reduced:false,sound:true};
...
test('dense-scene logs aggregate correct FPS, phase denominators and bounded line sizes',()=>{
 ...
 for(let i=1;i<=250;i++)p.frame(frame(i*20,{count:i<101?24:60,hudChanged:i%5===0,steps:2,
 ...
 assert.match(logs[0],/采样开始 v4.*我方=剪刀/);assert.match(logs[0],/连续网格索引/);
 assert.match(logs[1],/逻辑摊销\/步=0.500ms/);
...
test('stutter detail is rate limited and long pauses have a distinct diagnostic',()=>{
 ...
 p.stall(620);assert.match(logs.at(-1),/长停顿 v4.*620.00ms.*自动暂停/);
});
test('theme/layout changes end the old window before the new context; save timing includes async completion',()=>{
 ...
 assert.match(logs.at(-1),/场景变化 v4.*战场=640.0vp.*主题=深色/);
