...
const dir = new URL('../entry/src/main/ets/gamesNext/tenGarden/', import.meta.url);
...
function windowSetup() {
  ...
  const {TenWindow} = load(['TenModel','TenLayout','TenWindow'],'TenWindow',{window,display,console:quiet});
  const observer = new TenWindow();
...
function audioSetup() {
  ...
  const {TenAudio}=load(['TenModel','TenAudio'],'TenAudio',{media,audio,console:quiet});
  return {adapter:new TenAudio(),state,plays,stops,opened,closed,pools,context,pool,media,
...
await test('sounds wait for decode, obey global mute, throttle rapid duplicate effects and use clear volume',async()=>{
  ...
  assert.equal(t.opened.length,6);assert.equal(t.closed.length,0);
  t.adapter.play('select');assert.equal(t.plays.length,0);
  t.finishDecode();t.adapter.setEnabled(false);t.adapter.play('select');assert.equal(t.plays.length,0);
  t.adapter.setEnabled(true);t.adapter.play('select');t.adapter.play('select');await Promise.resolve();
  ...
  await t.adapter.release();assert.equal(t.closed.length,6);assert.equal(new Set(t.closed).size,6);
  assert.equal(t.pools.length,1);t.adapter.play('complete');assert.equal(t.plays.length,1);
});
await test('a play promise finishing after pause is stopped and audio creation finishing after exit is released',async()=>{
  ...
  t.adapter.play('collect');t.adapter.silence();finish(999);await Promise.resolve();
...
function storageSetup() {
  ...
  const preferences={async getPreferences(_context, options){
    assert.equal(options.name,'ten_garden_v1');
    return {
  ...
  const {TenStorage}=load(['TenStorage'],'TenStorage',{preferences,console:quiet});
  return {adapter:new TenStorage(),state,writes};
}
...
console.log('\nTen Garden: ' + count + ' platform adapter groups passed; physical-device behavior remains manual QA.');
