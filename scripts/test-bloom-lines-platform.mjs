...
const dir = new URL('../entry/src/main/ets/gamesNext/bloomLines/', import.meta.url);
...
function windowSetup() {
  ...
  const {BloomWindow} = load(['BloomModel','BloomLayout','BloomWindow'],'BloomWindow',{window,display,console:quiet});
  const observer = new BloomWindow();
...
function audioSetup() {
  ...
  const {BloomAudio}=load(['BloomModel','BloomAudio'],'BloomAudio',{media,audio,console:quiet});
  return {adapter:new BloomAudio(),state,plays,stops,opened,closed,pools,context,pool,media,
...
await test('a play promise finishing after pause is stopped and audio creation finishing after exit is released',async()=>{
  ...
  t.adapter.play('clear');t.adapter.silence();finish(999);await Promise.resolve();
...
function storageSetup() {
  ...
  const preferences={async getPreferences(_context, options){
    assert.equal(options.name,'bloom_lines_v1');
    return {
  ...
  const {BloomStorage}=load(['BloomStorage'],'BloomStorage',{preferences,console:quiet});
  return {adapter:new BloomStorage(),state,writes};
}
...
console.log('\nBloom Lines: ' + count + ' platform adapter groups passed; physical-device behavior remains manual QA.');
