...
const dir = new URL('../entry/src/main/ets/gamesNext/memoryGarden/', import.meta.url);
...
function windowSetup() {
  ...
  const {MemoryWindow} = load(['MemoryModel','MemoryLayout','MemoryWindow'],'MemoryWindow',{window,display,console:quiet});
  const observer = new MemoryWindow();
...
function audioSetup() {
  ...
  const {MemoryAudio}=load(['MemoryModel','MemoryAudio'],'MemoryAudio',{media,audio,console:quiet});
  return {adapter:new MemoryAudio(),state,plays,stops,opened,closed,pools,context,pool,media,
...
await test('sounds wait for decode, obey global mute, throttle rapid duplicate effects and use clear volume',async()=>{
  ...
  t.adapter.play('flip');assert.equal(t.plays.length,0);
  t.finishDecode();t.adapter.setEnabled(false);t.adapter.play('flip');assert.equal(t.plays.length,0);
  t.adapter.setEnabled(true);t.adapter.play('flip');t.adapter.play('flip');await Promise.resolve();
...
await test('a play promise finishing after pause is stopped and audio creation finishing after exit is released',async()=>{
  ...
  t.adapter.play('match');t.adapter.silence();finish(999);await Promise.resolve();
...
function storageSetup() {
  ...
  const preferences={async getPreferences(_context, options){
    assert.equal(options.name,'memory_pairs_v1');
    return {
  ...
  const {MemoryStorage}=load(['MemoryStorage'],'MemoryStorage',{preferences,console:quiet});
  return {adapter:new MemoryStorage(),state,writes};
}
...
console.log('\nMemory Garden: ' + count + ' platform adapter groups passed; physical-device behavior remains manual QA.');
